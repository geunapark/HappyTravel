...
//    500번대는 서버오류
    ...
    private static final Map<String, CustomErrorCode> errorCodeMap = new HashMap<>();
    static {
    ...
    public static CustomErrorCode findErrorCode(String code){
        CustomErrorCode errorCode = errorCodeMap.get(code);

        if (errorCode == null) {
            throw new CustomException(COMMON_ERROR_CODE_NOT_MATCH);
        }
        return errorCode;
    } // findErrorCode
